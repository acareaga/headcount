...
class StatewideTestingTest < Minitest::Test
  ...
  def test_grade_8_scores_by_subject
    ...
    assert_equal({2008=>{:math=>0.64, :reading=>0.843, :writing=>0.734},
                  ...
                  2014=>{:math=>0.684, :reading=>0.827, :writing=>0.747}}, district)
  end
  ...
  def test_proficient_for_subject_by_race_in_year
    ...
    repository = DistrictRepository.from_csv(path)
    district   = repository.find_by_name("ACADEMY 20")
                .statewide_testing.proficient_for_subject_by_race_in_year(:math, :asian, 2011)
    assert_equal 0.8976, district
  end
end
